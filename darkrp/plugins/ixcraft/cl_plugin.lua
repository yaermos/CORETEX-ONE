...
function PANEL:Init()
	...
	self.infolabel:SetText("Наведите на иконку чтобы узнать рецепт.")
...
function PANEL:Init()
	...
	function self.craftbutton:Paint(w2, h2)
		...
		draw.SimpleTextOutlined("Создать", "WastelandStandard", w2 - 100, h2 / 18, Color(255, 255, 255, 255), 5, 5, 1, Color(0, 0, 0, 255))
	end
...
function PANEL:SetItem(name, icon, desc, req, results, skill, blueprint, guns, entity)
	...
	self.labelitemreq:SetText("Нужно: " .. table.concat(realreqd, ", "))
	...
	self.spawnicon:SetHelixTooltip(function(tooltip)
		...
		requirements:SetText("Нужно: " .. table.concat(realreq, ", "))
		local missing = {}
		...
		results:SetText("Будет: " .. table.concat(realres, ", "))
		...
		if self.skill then
			...
			skill:SetText("Нужная способность: " .. table.concat(skillist, ", "))
			...
			local skill = tooltip:AddRow("skill")
			skill:SetText("Не нужно способносткй")
			skill:SizeToContents()
		end
		...
		if self.blueprint then
			...
			bp:SetText("Разблокируется с помощью чертёжа")
			bp:SizeToContents()
		end
	end)
