...
PLUGIN.name = "Улучшенные радио"
PLUGIN.author = "Ден"
PLUGIN.description = "Улучшенные радио."
...
if (CLIENT) then
	...
	netstream.Hook("Channel", function(names)
		...
		local quer = Derma_Query("Левый клик чтобы выбрать канал\nПравый клик чтобы изменить канал", "Channel selection",
		...
		function ch1panel:DoRightClick()
			Derma_StringRequest("Channel Name", "Какое будет новое имя?\nОставьте пустое место чтобы сбросить по умолчанию", names[1], function(text)
				if text == " " then 
					setTheName("1","CH1")
				else
					setTheName("1",text)
		...
		function ch2panel:DoRightClick()
			Derma_StringRequest("Channel Name", "Какое будет новое имя?\nОставьте пустое место чтобы сбросить по умолчанию", names[2], function(text)
				if text == " " then 
					setTheName("2","CH2")
				else
					setTheName("2",text)
		...
		function ch3panel:DoRightClick()
			Derma_StringRequest("Channel Name", "Какое будет новое имя?\nОставьте пустое место чтобы сбросить по умолчанию", names[3], function(text)
				if text == " " then 
					setTheName("3","CH3")
				else
					setTheName("3",text)
		...
		function ch4panel:DoRightClick()
			Derma_StringRequest("Channel Name", "Какое будет новое имя?\nОставьте пустое место чтобы сбросить по умолчанию", names[4], function(text)
				if text == " " then 
					setTheName("4","CH4")
				else
					setTheName("4",text)
	...
	netstream.Hook("Frequency", function(oldFrequency)
		Derma_StringRequest("Частота", "На какую частоту вы хотите изменить?", oldFrequency, function(text)
	...
	netstream.Hook("FrequencyMenu", function(oldFreqs)
		...
		Derma_Query("Выберите что хотите сделать", "Duplex Frequency Control",
		"Изменить частоту ("..oldTransmit.." MHz)",function()
			Derma_StringRequest("Frequency", "Текущая частота: "..oldTransmit.." MHz".."\nНа какую частоту вы хотите изменить?", oldTransmit, function(text)
			--repeater("I"..text)
			ix.command.Send("SetFreq", text)
			end)
		end,
		"Change Receiving Frequency ("..oldListen.." MHz)",function()
			Derma_StringRequest("Frequency", "текущая получающая частота: "..oldListen.." MHz".."\nНа какую частоту вы хотите изменить?", oldListen, function(text)
			--repeater("O"..text)
...
ix.config.Add("radioColor",Color(164,224,91), "Стандартный цвет радио-чата.", nil, {category = "Extended Radio"})
...
ix.config.Add("longRangeColor", Color(255,139,82), "Стандартный увет для длинного радио-чата.", nil, {category = "Extended Radio"})
ix.config.Add("walkieTalkieColor", Color(91,182,224), "Стандартный цвет для чата переносной рации.", nil, {category = "Extended Radio"})
...
ix.config.Add("radioFreqColor",Color(175,175,175), "Стандартный цвет радио частот.", nil, {category = "Extended Radio"})
ix.config.Add("activeFreqColor", Color(245,245,245), "Стандартный цвет для длинных частот.", nil, {category = "Extended Radio"})

ix.config.Add("beepVolumeMult", 1, "Увеличитель звука *Бип-Буп*.", nil, {
